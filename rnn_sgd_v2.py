...
import ngram
from octave_vecs_v2 import vec_list_for_song as vectorizer
...
class RNN_No_FFNN(nn.Module):
    ...
    def compute_Loss(self, predicted_vector, gold_label):
        pred_logsoft = torch.cat((
            torch.log_softmax(predicted_vector[:13], 0), #pitch
            torch.log_softmax(predicted_vector[13:15], 0), # duration
            torch.log_softmax(predicted_vector[15:], 0) # num_notes
        ))
        gold_soft = torch.softmax(gold_label.float(), 0)
        return self.loss(pred_logsoft, gold_soft)    
        # correct according to https://discuss.pytorch.org/t/kl-divergence-loss/65393/3

    def forward(self, line, hidden): 
        tensor = torch.from_numpy(line).float() 
        h_t = self.activation(self.U(hidden) + self.W(tensor))
        # return self.softmax(self.V(h_t)), h_t
...
if __name__ == "__main__":
    ...
    with open('rnn_models/Song_GD&octave_vecs&hidden_dim=30&learning_rate=0.25&1618103280&epoch=20&dist=12.615211486816406', 'rb') as f:
        model = torch.load(f)
    main(hidden_dim=hidden_dim_rnn, num_epochs=number_of_epochs, learning_rate=lr, existing_model=model, epoch_start=20)
